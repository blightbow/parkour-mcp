...
- **`fp_grid = (U8+F8_E8M0) / (U8+F8_E8M0 + BF16+F16)`** — share of scale metadata that is E8M0
  ...
  **Undefined, not 0.0, when the scales are not in the histogram.** The Hub does not guarantee
  ...
  the same model in the same format reports them normally. Gate on `storage_scoped` below.

  *A scale-cardinality floor (`scale_like ≥ payload / 128`) was tried here first and removed.
  It rests on the group size being bounded, which holds only for group-wise quantization:
  per-channel schemes put the group at `in_features` and per-tensor at the whole tensor. Across
  a 1,924-repo sweep it was the deciding gate for exactly one repo,
  `ixim/Qwen-Image-2512-Quanto-INT8-Full` at an effective group of ~1,835, and suppressed it
  wrongly. Do not reintroduce it.*
- **`storage_scoped = Σ(count × dtype width) < canonical_bytes × 1.35`** — the load-bearing
  gate, and the one to reach for. Scale arrays are quantization *metadata*,
  ...
  The ratio means "scoping" only while the canonical set is the whole of what the histogram
  describes, so precondition 2's partition is load-bearing here and not merely for `bpw`.
  Genuine duplicate sets are safe, since the histogram counts one copy; a checkpoint *split*
  across two sets is what breaks it.
