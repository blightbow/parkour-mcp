...
- **`youtube.py`** — YouTube integration via yt-dlp's unofficial Innertube extraction. 1 tool action so far (video — metadata + description); channel, playlist, transcript, and search actions land in subsequent commits. URL detection for `watch`, `youtu.be`, `shorts`, `clip`, `embed`, `@handle`, `/channel/UC...`, `/c/`, `/user/`, `/playlist`. `music.youtube.com` deliberately excluded (deferred sibling tool). Single shared `YoutubeDL` instance for video mode (PoToken cache and JS player solve are instance-scoped). Sync extraction wrapped via `asyncio.to_thread`. Bot-detection / private / age-restricted / geo-restricted errors mapped to user-facing strings. No auth required.
