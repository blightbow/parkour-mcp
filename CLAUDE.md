...
<!-- [[[end]]] -->
- **`hermes_plugin.py`** — Second package entrypoint: a native Hermes Agent plugin (declared as the `hermes_agent.plugins` entry point in `pyproject.toml`). Hermes calls `register()` once at startup and parkour's tools join the host registry alongside Hermes' built-ins. Chosen over the MCP transport because Hermes delivers a native tool's return string verbatim into model context, so parkour's frontmatter-first fenced output stays document-shaped instead of JSON-escaped. Runs one private long-lived event loop on a daemon thread (calls marshalled via `asyncio.run_coroutine_threadsafe`, handlers registered `is_async=False`) to preserve parkour's loop-bound module-global state (`RateLimiter` locks, the shelf lock, per-repo cache locks). Opt-in `override_web_extract` / `override_web_search` flags under `plugins.entries.parkour` in `~/.hermes/config.yaml` make parkour replace the host's built-in web tools. Backed by the `hermes` profile in `__init__.py#PROFILE_VARS`.
- **`detection.py`** — Pure, stdlib-only URL/identifier classification: the `_detect_*_url` predicates, regexes, ID extractors, and `RedditPageType` for arXiv, DOI, Semantic Scholar, IETF, and Reddit. Imported at module top by `fetch_direct.py`, `_pipeline.py`, and `kagi.py` (and by the source modules for their own internal use). Kept dependency-free so the fast-path dispatchers and sibling tools can classify a URL without loading a source module's transport stack (httpx, curl_cffi). GitHub detection stays in `github.py` (it consults `_get_github_token()`, so it is not stateless); MediaWiki and Discourse detect at fetch time, not from the URL.
...
- **`markdown.py`** — HTML→markdown conversion. The generic path uses the Rust-backed `htmd` library (`html_to_markdown` calls `htmd.convert_html`); the markdownify-based `TextOnlyConverter` is retained only for the MediaWiki path (`_mediawiki_html_to_markdown`), with BeautifulSoup kept for small head-only parses (title extraction, JS-shell detection). Section extraction with fuzzy slug matching. Content fencing. Semantic truncation for markdown, hard truncation for structured formats.
...
- **`kagi.py`** — Search via the v1 API over httpx with Bearer auth, supporting `workflow` (search/images/videos/news/podcasts), `lens_id`, `page`, and flat `region`/`after`/`before` filters that assemble into v1's `filters` object. Local **query presets** (`preset` arg, `kagi://presets` resource, `kagi_presets.yaml` registry under the config dir) bundle a query-operator `fragment` (an unbounded `site:` allow-list, unlike a lens's 10-site cap) plus structured filters; the fragment is auto-grouped (a top-level `OR` is parenthesized via `_group_fragment`, reusing `_has_ungrouped_or`) and prefixed to the query, an explicit arg overrides a preset value (with a frontmatter warning), and an unknown name errors rather than silently falling back. `parkour-mcp --init` seeds a commented starter registry (creating the config dir via `common.py#ensure_dir`) and never clobbers; it is reachable on bundle installs too (the extension is an on-disk venv), and the `kagi://presets` resource prints the exact `sys.executable`-based command alongside the `_PRESETS_TEMPLATE`. See `docs/kagi-query-presets.md`. The `kagi_summarize` tool is unregistered pending Kagi shipping `/summarize` on v1; the backing `summarize()` function, the `kagiapi` dep, and the v0 balance-lockout helpers (`get_client`, `_extract_balance`, `_check_balance`, `_summarize_locked`, `_handle_v0_error`) stay dormant in this file for re-registration. v1 spec lives under `https://kagi.com/api/docs/`: overview `openapi.md`, search `openapi/search.md` and operation `openapi/search/search.md`, extract `openapi/extract.md` and operation `openapi/extract/extractcontent.md`, full bundles `_bundle/openapi.yaml?download` and `_bundle/openapi.json?download`. Fetch via `curl`; `web_fetch_direct` currently rejects `text/markdown` and `application/yaml` content types.
...
- **`semantic_scholar.py`** — S2 API with optimized field sets per query type. 1s client-side rate limit, applied unconditionally; `S2_API_KEY` adds the `x-api-key` header for a higher server-side quota but does not change the local limiter.
- **`doi.py`** — DOI resolution via content negotiation. Registration agency detection. DataCite enrichment (ORCID, licenses, related identifiers, resource type). Author affiliations are not pulled from the DataCite REST path.
...
**Profiles**: The server registers its tools under different naming conventions depending on the `--profile` flag. `code` uses PascalCase (`WebFetchIncisive`), `desktop` uses snake_case (`web_fetch_incisive`). Tool descriptions also adapt: they reference sibling tools by their profile-appropriate names. A third `hermes` profile backs the Hermes Agent plugin (`hermes_plugin.py`); it reuses `desktop`'s snake_case tool names but supplies its own description placeholders pointing at Hermes' built-in `web_extract` / `web_search`.
...
| `MCP_CONTACT_EMAIL` | Adds a `mailto` to CrossRef requests, enrolling in the server-side "polite pool". The local CrossRef limiter stays fixed at 5 req/s regardless. |
...
- Live integration tests carry the `live` marker (`test_live.py`, `test_github_live.py`) and are deselected by default; run with `-m live`. A `perf` marker is likewise excluded by default.
