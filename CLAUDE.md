...
- **`_trace.py`** — Wire trace. Both `guarded_fetch` functions record each exchange (URL as sent, request headers, status, locating response headers, elapsed, the `RateLimiter` that governed it) as one `Exchange`; `PARKOUR_TRACE` selects a JSON-lines sink, `collect()` gathers records in-process, `dry_run()` stops a fetch after recording, and `render_curl` turns a record into a curl command behind its rate-limit contract. A witness, not a knob.
- **`_cli.py`** — `parkour-mcp call`, which runs one registered tool through the production path from the shell, with `--trace`, `--dry-run`, and `--as-curl`. Audience and rules: `docs/developing.md#debugging-live-behavior`.
