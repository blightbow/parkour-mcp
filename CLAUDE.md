...
# Invoke one tool through the production path; see docs/developing.md#debugging-live-behavior
uv run parkour-mcp call ArXiv '{"action": "search", "query": "ti:attention"}' --as-curl
```
...
- **`_trace.py`** — Wire trace. Both `guarded_fetch` functions record each exchange (URL as sent, request headers, status, locating response headers, elapsed, the `RateLimiter` that governed it) as one `Exchange`; `PARKOUR_TRACE` selects a JSON-lines sink, `collect()` gathers records in-process, `dry_run()` stops a fetch after recording, `capture()` keeps full responses for `write_fixture`, and `render_curl` turns a record into a curl command behind its rate-limit contract. A witness, not a knob; the one knob, `common.py#force_http1`, lives beside the transports it governs.
- **`_cli.py`** — `parkour-mcp call`, which runs one registered tool through the production path from the shell, with `--trace`, `--dry-run`, `--as-curl`, `--http1`, and `--record` (a fixture `tests/_replay.py` mounts as respx routes). Audience and rules: `docs/developing.md#debugging-live-behavior`.

API integration modules, each self-contained:
- **`kagi.py`** — Search via the v1 API over httpx with Bearer auth, supporting `workflow` (search/images/videos/news/podcasts), `lens_id`, `page`, and flat `region`/`after`/`before` filters that assemble into v1's `filters` object. The `kagi_summarize` tool is unregistered pending Kagi shipping `/summarize` on v1; the backing `summarize()` function, the `kagiapi` dep, and the v0 balance-lockout helpers (`get_client`, `_extract_balance`, `_check_balance`, `_summarize_locked`, `_handle_v0_error`) stay dormant in this file for re-registration. v1 spec lives under `https://kagi.com/api/docs/`: overview `openapi.md`, search `openapi/search.md` and operation `openapi/search/search.md`, extract `openapi/extract.md` and operation `openapi/extract/extractcontent.md`, full bundles `_bundle/openapi.yaml?download` and `_bundle/openapi.json?download`. Both fetch through `web_fetch_direct`: the `.md` pages as sectioned markdown, the YAML bundle raw.
- **`fetch_direct.py`** — The `web_fetch_direct` tool (WebFetchIncisive). Static HTTP fetching with content-type detection. Routes URLs through the fast-path chain; falls back to static HTTP, or to the headless-browser renderer when `requires_js=True` / `actions` is set. Non-HTML bodies are classified by `_pipeline.py#_classify_body` (Content-Type first, then `%PDF-` magic) and rendered by `_pipeline.py#_render_body`, shared with the JS path: PDF and markdown are document kinds that enter the section, slice, and search pipeline; JSON, XML, YAML, and plain text are surfaced raw. A URL shaped like a PDF (`detection.py#_detect_pdf_url`) takes the 50 MiB sections ceiling because its output is bounded by `max_tokens` after decoding. A targeted arXiv request whose `/html/` rendering is missing falls back to the paper's PDF (`_arxiv_pdf_fallback`), cached under the `/html/` key so the `/abs/` rewrite keeps hitting it.
- **`pdf.py`** — PDF to markdown via `pdf-inspector` (pure Rust, MIT, pinned). `pdf_to_markdown` returns a `PdfDocument` (title, author, page count, markdown) and repairs two artifacts of the decoder's font-size heading tiers from its positioned text rather than its markdown: runs rotated relative to their page (the arXiv margin stamp, watermarks) are dropped instead of landing as a heading mid-paragraph, and a heading that is a display equation or over 120 characters is demoted to prose. A page whose text is predominantly rotated keeps its runs. The engine choice, the PyMuPDF licence verdict, and the measured comparison live in `.claude/TECH_DEBT.md#applicationpdf-support-landed-page-rendering-deferred`.
...
| `PARKOUR_TRACE` | Wire trace sink: a file path, or `-` for stderr. One JSON line per outbound exchange (default: off) |
...
- Reddit fast path authenticates with a *userless* OAuth token against `oauth.reddit.com`, minted via Reddit's own logged-out mobile-app grant (mirroring redlib). It uses `wreq` with a Safari emulation profile (`_EMULATION`) to clear Reddit's JA3 filter, plus spoofed Android device headers on the token mint. This is the userless/anonymous OAuth lane (no user account, no API key, no app registration), NOT a registered-app integration. The unauthenticated `.json` endpoints it formerly used were retired by Reddit on 2026-05-29.
